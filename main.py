...
def gram_schmidt(matrix):
    ...
    for idx, col_vector in enumerate(matrix.T):
        if idx == 0:
            magnitude = np.linalg.norm(col_vector)
            if magnitude == 0:
                new_orthogonal_vector = np.zeros(len(col_vector))
            else:
                new_orthogonal_vector = col_vector / magnitude
            ...
            magnitude = np.linalg.norm(new_orthogonal_vector)
            if magnitude == 0:
                new_orthogonal_vector = np.zeros(len(col_vector))
            else:
                new_orthogonal_vector = new_orthogonal_vector / magnitude
            orthogonal_vectors.append(new_orthogonal_vector)
